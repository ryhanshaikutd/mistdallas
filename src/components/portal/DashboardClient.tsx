...
function greeting(name: string, hour: number) {
  return `${hour < 12 ? "Good morning" : hour < 17 ? "Good afternoon" : "Good evening"}, ${name}`;
}

function WeekView({ tasks, milestones }: { tasks: Task[]; milestones: Milestone[] }) {
  const [weekOffset, setWeekOffset] = useState(0);
  // null until mounted to avoid hydration mismatch
  ...
  if (!today || !selectedDay) {
    return (
      <div className="rounded-2xl border overflow-hidden" style={{ background: "var(--p-card)", borderColor: "var(--p-border)", minHeight: 220 }}>
        <div className="px-5 py-4 border-b" style={{ borderColor: "var(--p-border)" }}>
          <div className="h-4 w-32 rounded-lg animate-pulse" style={{ background: "var(--p-card-hover)" }} />
        </div>
        <div className="grid grid-cols-7 border-b" style={{ borderColor: "var(--p-border)" }}>
          {Array.from({ length: 7 }).map((_, i) => (
            <div key={i} className="flex flex-col items-center py-4 gap-2">
              <div className="h-3 w-6 rounded animate-pulse" style={{ background: "var(--p-card-hover)" }} />
              <div className="w-8 h-8 rounded-full animate-pulse" style={{ background: "var(--p-card-hover)" }} />
            </div>
          ))}
        </div>
        <div className="p-5">
          <div className="h-3 w-40 rounded animate-pulse mb-4" style={{ background: "var(--p-card-hover)" }} />
        </div>
      </div>
    );
  }
    ...
    <div className="rounded-2xl border overflow-hidden" style={{ background: "var(--p-card)", borderColor: "var(--p-border)" }}>
      ...
      <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: "var(--p-border)" }}>
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setWeekOffset(w => w - 1)} className="p-1.5 rounded-lg transition-colors"
          ...
          <button
            ...
            style={{ color: "var(--p-accent)", background: isCurrentWeek ? "var(--p-accent)" + "15" : "transparent" }}>
            Today
          </button>
          <button onClick={() => setWeekOffset(w => w + 1)} className="p-1.5 rounded-lg transition-colors"
            style={{ color: "var(--p-muted)" }}
      ...
      <div className="grid grid-cols-7 border-b" style={{ borderColor: "var(--p-border)" }}>
        {days.map(day => {
          ...
          const isNow = isCurrentWeek && isSameDay(day, today);
          ...
          return (
            <button key={day.toISOString()} onClick={() => setSelectedDay(day)}
              ...
              style={{ borderColor: "var(--p-border)", background: selected ? "var(--p-accent)" + "12" : "transparent" }}>
              ...
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mb-2"
                style={{
                  background: isNow ? "#1B3464" : selected ? "var(--p-accent)" + "20" : "transparent",
                  color: isNow ? "#ffffff" : selected ? "var(--p-accent)" : "var(--p-text)",
              ...
              {total > 0 ? (
                <div className="flex gap-0.5 flex-wrap justify-center">
                  {dayMilestones.slice(0, 2).map(m => (
                    <div key={m.id} className="w-1.5 h-1.5 rounded-full" style={{ background: "#2EA87A" }} />
                  ))}
        ...
        {selectedTasks.length === 0 && selectedMilestones.length === 0 ? (
          ...
          <div className="space-y-2">
            {selectedMilestones.map(m => (
              <div key={m.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: "#2EA87A15" }}>
                <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: "#2EA87A" }} />
                <span className="text-sm font-semibold" style={{ color: "#2EA87A" }}>{m.title}</span>
                <span className="text-xs ml-auto" style={{ color: "#2EA87A80" }}>Milestone</span>
              </div>
            ))}
            {selectedTasks.map(t => (
...
export default function DashboardClient({ profile, recentApplications, pendingTasks, milestones, activeCycle }: Props) {
  const [hour, setHour] = useState(12); // neutral default during SSR
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6" style={{ animation: "fadeInUp 0.4s ease both" }}>
      {/* Greeting */}
      <div>
        <h2 className="text-3xl font-extrabold" style={{ fontFamily: "var(--font-syne)", color: "var(--p-text)" }}>
          {greeting(displayName, hour)} 👋
        </h2>
          ...
          {quickLinks.map(l => {
            ...
            return (
              <Link key={l.href} href={l.href}
                ...
                style={{ background: "var(--p-card)", borderColor: "var(--p-border)", boxShadow: "var(--p-shadow)" }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "var(--p-accent)"; (e.currentTarget as HTMLElement).style.boxShadow = "var(--p-shadow-md)"; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "var(--p-border)"; (e.currentTarget as HTMLElement).style.boxShadow = "var(--p-shadow)"; }}>
                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: l.color + "18" }}>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        {/* Applications */}
        <div className="rounded-2xl border overflow-hidden" style={{ background: "var(--p-card)", borderColor: "var(--p-border)", boxShadow: "var(--p-shadow)" }}>
          <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: "var(--p-border)" }}>
